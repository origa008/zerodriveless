          ...
          <div className="mb-6">
            <div className="border border-gray-200 rounded-2xl p-4">
              ...
              <div className="flex items-center space-x-4">
                <div 
                  className={`flex items-center p-2 border rounded-lg ${paymentMethod === 'cash' ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}
                  onClick={() => setPaymentMethod('cash')}
                >
                  <div className={`w-4 h-4 rounded-full mr-2 ${paymentMethod === 'cash' ? 'bg-blue-500' : 'border border-gray-400'}`}></div>
                  <span>Cash</span>
                </div>
                <div 
                  className={`flex items-center p-2 border rounded-lg ${paymentMethod === 'wallet' ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}
                  onClick={() => setPaymentMethod('wallet')}
                >
                  <div className={`w-4 h-4 rounded-full mr-2 ${paymentMethod === 'wallet' ? 'bg-blue-500' : 'border border-gray-400'}`}></div>
                  <span>Wallet (RS {walletBalance.toFixed(0)})</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            ...
            <div className="flex space-x-3">
              ...
              <Button 
                ...
                disabled={bidAmount < calculateBaseFare(estimatedDistance, selectedRideOption.name) || (paymentMethod === 'wallet' && bidAmount > walletBalance)}
              >
                Place Bid
              </Button>
            </div>
